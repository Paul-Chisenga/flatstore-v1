<x-admin.root>
    <x-admin.page-header title="Create Category" description="Manage your store's categories." />
    ...
    <x-ui.card class="mt-8">
        ...
        <x-ui.card.card-content>
            <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-6">
                ...
                <x-ui.field.field>
                    <x-ui.field.field-label for="icon">Icon Name</x-ui.field.field-label>
                    <x-ui.input id="icon" name="icon" :value="old('icon')" />
                    <x-ui.field.field-error :messages="$errors->get('icon')" />
                </x-ui.field.field>
                <x-ui.field.field>
